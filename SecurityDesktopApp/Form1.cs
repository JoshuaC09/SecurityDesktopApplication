...
namespace SecurityDesktopApp
{
    public partial class EncryptForm : Form
    {
        ...
        private readonly byte[] salt = new byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f }; // This is a fixed salt for the example. You should generate and store this securely.
        public EncryptForm()
        {
            InitializeComponent();
        ...
        private void btnEncrypt_Click_1(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(userName) && !string.IsNullOrWhiteSpace(password))
            {
                try
                {
                    // Create an instance of the StringEncryptionService with a password and salt
        ...
        private void btnDecryptionForm_Click(object sender, EventArgs e)
        {
            ...
            {
                decryptForm = new DecryptForm(); // Create a new instance if needed
                decryptForm.EncryptForm = this; // Pass a reference to the current EncryptForm
            }
